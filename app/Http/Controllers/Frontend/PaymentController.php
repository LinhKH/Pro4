...
class PaymentController extends Controller
{
    ...
    public function paypalSuccess(Request $request)
    {
        ...
        if (isset($response['status']) && $response['status'] == 'COMPLETED') {
            ...
            $orderId = $this->storeOrder('paypal', 1, $response['id'], $paidAmount, $paypalSetting->currency_name);
    ...
    public function payWithStripe(Request $request)
    {
       ...
       $response = Charge::create([
        ...
        if($response->status === 'succeeded'){
            $orderId = $this->storeOrder('stripe', 1, $response->id, $payableAmount, $stripeSetting->currency_name);
    ...
    public function payWithRazorPay(Request $request)
    {
       ...
       if($request->has('razorpay_payment_id') && $request->filled('razorpay_payment_id')){
            ...
            if($response['status'] == 'captured'){
                $orderId = $this->storeOrder('razorpay', 1, $response['id'], $payableAmount, $razorPaySetting->currency_name);
    ...
    public function payWithCod(Request $request)
    {
       ...
       $payableAmount = round($total, 2);
        ...
        $orderId = $this->storeOrder('COD', 0, \Str::random(10), $payableAmount, $setting->currency_name);
